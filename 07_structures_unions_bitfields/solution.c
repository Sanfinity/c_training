/*
 * Topic 07: Structures, Unions & Bit-fields
...
long by_value(struct Point p)        { return (long)p.x + p.y; }
long by_pointer(const struct Point *p){ return (long)p->x + p->y; }
...
static uint32_t set_speed(uint32_t reg, uint32_t speed) {
    return (reg & ~SPEED_MASK) | ((speed << SPEED_SHIFT) & SPEED_MASK);
}
