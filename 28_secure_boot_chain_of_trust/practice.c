/*
 * Topic 28: Secure Boot & Chain of Trust -- PRACTICE TEMPLATE
...
uint64_t compute_hash(const struct Image *img) {
    /* TODO: implement. HINT: hash just the code -- return fnv1a_64(img->data, img->len).
     *       Do NOT fold in img->version (an old image must stay verifiable). */
    (void)img;
    return 0; /* placeholder */
}
...
uint64_t sign_hash(uint64_t hash) {
    /* TODO: implement. HINT: the toy signature is XOR with the secret -- return hash ^ ROOT_KEY. */
    (void)hash;
    return 0; /* placeholder */
}
...
int ct_eq_u64(uint64_t a, uint64_t b) {
    /* TODO: implement. HINT: d = a ^ b; OR it down (d |= d>>32; >>16; >>8; >>4; >>2; >>1);
     *       return 1 only if no bits are set: (~d & 1). No branches on the data. */
    (void)a; (void)b;
    return 0; /* placeholder */
}
...
int verify_signature(const struct Image *img, uint64_t expected_sig) {
    /* TODO: implement. HINT: re-sign the recomputed hash then constant-time compare:
     *       ct_eq_u64(sign_hash(compute_hash(img)), expected_sig). */
    (void)img; (void)expected_sig;
    return 0; /* placeholder */
}
...
int verify_stage(const struct Image *next, uint64_t expected_sig) {
    /* TODO: implement. HINT: the stage gate is just the signature check here --
     *       return verify_signature(next, expected_sig). */
    (void)next; (void)expected_sig;
    return 0; /* placeholder */
}
...
int anti_rollback(uint32_t version, uint32_t *fuse_min_version) {
    /* TODO: implement. HINT: if (version < *fuse_min_version) return 0;
     *       else { *fuse_min_version = version; return 1; }  (monotonic raise). */
    (void)version; (void)fuse_min_version;
    return 0; /* placeholder */
}
...
int key_revoked(unsigned key_slot, uint32_t revoke_fuses) {
    /* TODO: implement. HINT: test the fuse bit -- return (revoke_fuses >> key_slot) & 1. */
    (void)key_slot; (void)revoke_fuses;
    return 0; /* placeholder */
}
...
uint64_t pcr_extend(uint64_t pcr, uint64_t measurement) {
    /* TODO: implement. HINT: serialize pcr then measurement into a 16-byte buffer
     *       (8 bytes each) and return fnv1a_64 of it = H(pcr || measurement). */
    (void)pcr; (void)measurement;
    return 0; /* placeholder */
}
...
size_t boot_chain(const struct Image *stages, const uint64_t *golden_sig,
    ...
    /* TODO: implement. HINT: loop i in 0..n-1: ok = verify_stage(&stages[i], golden_sig[i]);
     *       results[i] = ok; if (!ok) break;  *pcr = pcr_extend(*pcr, compute_hash(&stages[i]));
     *       count the stages that pass and return the count. */
    (void)stages; (void)golden_sig; (void)n; (void)pcr; (void)results;
    return 0; /* placeholder */
}
...
void tamper_byte(uint8_t *buf, size_t idx) {
    /* TODO: implement. HINT: flip one bit -- buf[idx] ^= 0x01. */
    (void)buf; (void)idx;
}
