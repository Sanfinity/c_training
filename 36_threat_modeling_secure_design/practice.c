/*
 * Topic 36: Threat Modeling & Secure Design -- PRACTICE TEMPLATE
...
unsigned dread_score(const struct Threat *t) {
    /* TODO: implement. HINT: sum the five DREAD fields and divide by 5 to get a
     *       single score on the same 0..10 scale. */
    (void)t;
...
uint64_t fnv1a_64(const void *data, size_t n) {
    /* TODO: implement. HINT: h = 0xcbf29ce484222325; for each byte: h ^= byte;
     *       h *= 0x100000001b3 (the FNV prime). */
    (void)data; (void)n;
    return 0; /* placeholder */
}
...
int ct_eq_bytes(const uint8_t *a, const uint8_t *b, size_t n) {
    /* TODO: implement. HINT: OR (a[i]^b[i]) for ALL n bytes into one accumulator
     *       (no early break); return accumulator == 0. */
...
uint64_t rot_derive(uint64_t root_seed, const char *label) {
    /* TODO: implement. HINT: pack the 8 root_seed bytes then the label bytes into
     *       one buffer and return fnv1a_64 of it = KDF(root || label). */
    (void)root_seed; (void)label;
    return 0; /* placeholder */
}
...
int verify_image(const struct Image *img, uint64_t signing_key) {
    /* TODO: implement. HINT: expected = fnv1a_64(code,len) ^ signing_key; then
     *       constant-time compare its bytes against img->signature (ct_eq_bytes). */
    (void)img; (void)signing_key;
    return 0; /* placeholder */
}
...
int anti_rollback(uint32_t version, uint32_t *fuse_min) {
    /* TODO: implement. HINT: if (version < *fuse_min) return 0; else raise the
     *       fuse (*fuse_min = version) and return 1. */
    (void)version; (void)fuse_min;
    return 0; /* placeholder */
}
...
int authorize(enum Role role, enum Op op) {
    /* TODO: implement. HINT: start allow = 0 (deny); flip to 1 only for explicit
     *       pairs -- vendor: any op; operator: status/update; guest: status. */
    (void)role; (void)op;
    return 0; /* placeholder */
}
...
int has_capability(unsigned have, unsigned need) {
    /* TODO: implement. HINT: allow only when every needed bit is present --
     *       mask 'have' by 'need' and check it equals 'need'. */
    (void)have; (void)need;
    return 0; /* placeholder */
}
...
int validate_descriptor(const struct UpdateDescriptor *d) {
    /* TODO: implement. HINT: reject code_len == 0 or > MAX_IMAGE_LEN, slot > 1,
     *       and version == 0; otherwise accept. Check every field before use. */
    (void)d;
    return 0; /* placeholder */
}
...
enum OtaState ota_step(enum OtaState s, enum OtaEvent e) {
    /* TODO: implement. HINT: switch on the state; advance ONLY on the one expected
     *       event, and default every other (state,event) pair to OTA_ABORT. */
    (void)s; (void)e;
    return OTA_ABORT; /* placeholder (fail-closed) */
}
...
int debug_unlock(enum Lifecycle lc, const uint8_t *token, size_t token_len,
    ...
    /* TODO: implement. HINT: deny outright if lc == LC_PRODUCTION; else require
     *       token_len == expected_len and a constant-time match of the bytes. */
    (void)lc; (void)token; (void)token_len; (void)expected; (void)expected_len;
    return 0; /* placeholder */
}
...
void secure_scrub(void *p, size_t n) {
    /* TODO: implement. HINT: write 0 through a volatile uint8_t* so the compiler
     *       cannot delete the wipe as a dead store. No return value. */
    (void)p; (void)n;
}
